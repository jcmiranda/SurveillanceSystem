...
bool BgdCapturerSingle::runInThread() {
   ...
   // TODO: add some means of signaling background thread to exit 
    ...
    for(;;) {
        ...
        if( (rc2 = pthread_rwlock_rdlock(
                        _frame_buffer[_cur_frame_i+1].rw_lock)) != 0) {
            perror("Unable to acquire 2nd read lock in BgdCapturerSingle");
        }

        if(difftime(_frame_buffer[_cur_frame_i+1].timestamp, // end
                    _frame_buffer[_cur_frame_i].timestamp)
                > 0) { // beg
            inc_cur_frame = true;
            std::cout << "inc" << std::endl;
        }

        if( (rc1 = pthread_rwlock_unlock(
                        _frame_buffer[_cur_frame_i].rw_lock)) != 0) {
            perror("Unable to release read lock in BgdCapturerSingle");
        } 
        
        if( (rc2 = pthread_rwlock_unlock(
                        _frame_buffer[_cur_frame_i].rw_lock)) != 0) {
            perror("Unable to release read lock in BgdCapturerSingle");
        }
